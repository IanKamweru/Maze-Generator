...
public class MazeGenerator {
    ...
    public void createMaze(Wall[] walls, Cell[][] mazeMap) {
		...
		//make set for each of the cells individually
		for (Cell[] cells : mazeMap) {
			for (Cell cell : cells) {
				unionfind.makeset(cell);
			}
		}
		...
		//generate wall index at random
		int wallIndex=(int)(Math.random() * walls.length);

		//while the cells are not all in one set, keep removing walls
		while(!isComplete(unionfind,mazeMap)){

			if(walls[wallIndex].visible) {
				if (walls[wallIndex].first != null && walls[wallIndex].second != null) {
					Cell first = walls[wallIndex].first;
					Cell second = walls[wallIndex].second;

					if (first.head != second.head) {
						walls[wallIndex].visible = false;
						unionfind.union(first, second);
					}
				}
			}

			wallIndex=(int)(Math.random() * walls.length);
		}


    }

    //helper to check whether all cells are in one set
    private boolean isComplete(UnionFind unionfind,Cell[][] mazeMap){
    	LLAddOnly arbitraryHead=mazeMap[0][0].head;

		for (Cell[] cells : mazeMap) {
			for (Cell cell : cells) {
				if (cell.head != arbitraryHead) return false;
			}
		}
